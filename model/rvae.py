...
class RVAE(nn.Module):
    ...
    def latent_sampler(self, batch_loader):
        def sample_z(indexes, use_cuda):
            input = batch_loader.next_batch(batch_size=None, target_str='train', custom_indexes=indexes)
    ...
    def sample(self, batch_loader, seq_len, seed, use_cuda, rnd = False):
        ...
        for i in range(seq_len):
            logits, initial_state, _, _ = self(0., None, None,
                                            decoder_word_input, decoder_character_input,
                                            seed, initial_state)
            ...
            if rnd:
                ...
                word = batch_loader.decode_word(prediction.data.cpu().numpy()[-1].argmax())
